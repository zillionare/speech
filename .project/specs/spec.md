...
>
> **关键架构决策：ASR 完全内嵌到 Python 服务进程**（`tts_service/live/asr_engine.py`）。
...
| AI_SPEAKING | RECORDING, WAITING_TRIGGER, FINISHED, ERROR |
| RECORDING | WAITING_TRIGGER, ERROR |
| WAITING_TRIGGER | AI_SPEAKING, RECORDING, FINISHED, ERROR |
| FINISHED | (无) |
| ERROR | (无) |
...
class StreamingTTSProxy:
    def __init__(self, engine: BaseEngine, sample_rate: int = 24000):
        self.engine = engine
        self.sr = sample_rate
...
- WAV 格式的 PCM chunk（200 ms 片段，第一帧带 44 字节 header，之后裸 PCM）

**Client → Server** Binary:
- 16 kHz mono PCM（裸字节流，无 WAV header）。每 1000 ms 一帧。
...
### 10.2 WAV 头生成
```python
def write_wav(path: Path, pcm_bytes: bytes, sample_rate: int = 16000, channels: int = 1):
    import wave
...
| **M2** | SPEC-004 — LiveSession 状态机 + 端点 | `/api/.../live/start/stop` 在 mock 数据下走通 |
| **M3** | SPEC-005, 006, 011 — 流式 TTS + WS + 前端播放 | 浏览器看到 AI 段音频在 ≤ 1 s 内开始 |
